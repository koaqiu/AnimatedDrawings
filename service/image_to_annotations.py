...
def image_to_annotations(img_fn: str, out_dir: str, service:str) -> None:
    """
    ...
    # resp = requests.post("http://47.100.250.153:8080/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    resp = requests.post(service + "/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    resp = requests.post(service + "/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    if resp is None or resp.status_code >= 300:
